...
export class EventsService {
  ...
  private getEvents(): Observable<IEvent[]> {
    return this.http.get('/api/events').map((response: Response) => {
      return <IEvent[]>response.json()
    }).catch(this.handleError)
  }

  private getEvent(id: number): Observable<IEvent> {
    return this.http.get('/api/events/' + id).map((response: Response) => {
      return <IEvent>response.json()
    }).catch(this.handleError)
  }

  private saveEvent(event): Observable<IEvent> {
  ...
  private searchSessions(searchTerm: string){
...
//       onlineUrl: 'http://www.angularConnect.com',
//       price: 599.99,
//       sessions: [
//         {
//           id: 1,
